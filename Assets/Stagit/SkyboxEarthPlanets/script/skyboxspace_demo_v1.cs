...
public class skyboxspace_demo_v1 : MonoBehaviour
{
	...
	void Start ()
	{
		RenderSettings.skybox = skyBoxMaterial [4]; 
	...
	void Update ()
	{
        NextSceneMoonVal=GazeInteractionNextMoonScene.number;
        
        RenderSettings.skybox = skyBoxMaterial[NextSceneMoonVal];
     //   topText = skyBoxMaterial[NextSceneMoon].name;
        sun.transform.eulerAngles = sunPosition[NextSceneMoonVal];
...
/**
    ...
    **/
       ...
       if (string.IsNullOrEmpty (topText) == false) {
           DrawText ("Showing moon from angle "+Angle, TextAnchor.UpperCenter, 150);
       }
